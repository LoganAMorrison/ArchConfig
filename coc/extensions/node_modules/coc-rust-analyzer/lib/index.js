...
async function activate(context) {
    ...
    if (!bin) {
        ...
        let ret = 0;
        if (config.prompt) {
            ret = await coc_nvim_1.workspace.showQuickpick(['Yes', 'Cancel'], msg);
        }
        if (ret === 0) {
            try {
...
function run(ctx) {
    return async () => {
        ...
        terminal = await coc_nvim_1.workspace.createTerminal(opt);
...
function debugSingle() {
    return async (runnable) => {
        ...
        const debugConfig = coc_nvim_1.workspace.getConfiguration('rust-analyzer.debug');
        if (debugConfig.get('runtime') == 'termdebug') {
            await coc_nvim_1.workspace.nvim.command(`TermdebugCommand ${executable} ${executableArgs}`);
            return;
        }
        if (debugConfig.get('runtime') == 'vimspector') {
            const name = coc_nvim_1.workspace.getConfiguration('rust-analyzer.debug.vimspector.configuration').get('name');
            const configuration = { configuration: name, Executable: executable, Args: executableArgs };
            await coc_nvim_1.workspace.nvim.call('vimspector#LaunchWithSettings', configuration);
            return;
        }
        throw new Error(`Invalid debug runtime: ${debugConfig.get('runtime')}`);
    };
}
...
function explainError(ctx) {
    return async () => {
        ...
        if (!ctx_1.isRustDocument(document))
            return;
        const diagnostic = (_b = (_a = ctx.client.diagnostics) === null || _a === void 0 ? void 0 : _a.get(coc_nvim_1.workspace.uri)) === null || _b === void 0 ? void 0 : _b.find((diagnostic) => isInRange(diagnostic.range, position));
        if (diagnostic === null || diagnostic === void 0 ? void 0 : diagnostic.code) {
            const explaination = child_process_1.spawnSync('rustc', ['--explain', `${diagnostic.code}`], { encoding: 'utf-8' }).stdout;
            const docs = [];
            let isCode = false;
            ...
            const factory = new coc_nvim_1.FloatFactory(coc_nvim_1.workspace.nvim, coc_nvim_1.workspace.env);
...
async function applySnippetWorkspaceEdit(edit) {
    var _a;
    if (!((_a = edit === null || edit === void 0 ? void 0 : edit.documentChanges) === null || _a === void 0 ? void 0 : _a.length)) {
        return;
    }
...
function openCargoToml(ctx) {
    return async () => {
        ...
        if (!location)
            return;
        await coc_nvim_1.workspace.jumpTo(location.uri);
    };
}
...
class Ctx {
    ...
    async checkUpdate(auto = true) {
        ...
        if (this.config.prompt) {
            ret = await coc_nvim_1.workspace.showQuickpick(['Yes, download the latest rust-analyzer', 'Check GitHub releases', 'Cancel'], msg);
        }
        if (ret === 0) {
            if (process.platform === 'win32') {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.openCargoToml = exports.openDocs = exports.ssr = exports.inlayHints = exports.runnables = exports.onEnter = exports.joinLines = exports.resolveCodeAction = exports.parentModule = exports.matchingBrace = exports.expandMacro = exports.syntaxTree = exports.reloadWorkspace = exports.status = exports.memoryUsage = exports.analyzerStatus = void 0;
...
exports.openCargoToml = new lc.RequestType('experimental/openCargoToml');
...
class HintsUpdater {
    constructor(ctx) {
        ...
        this.disposables = [];
        this.inlayHintsNS = coc_nvim_1.workspace.createNameSpace('rust-inlay-hint');
    ...
    async renderHints(doc, hints) {
        ...
        if (this.ctx.config.inlayHints.typeHints) {
            ...
            for (const item of decorations.type) {
                const chunks = [[`${sep}${item.label}`, 'CocRustTypeHint']];
                doc.buffer.setVirtualText(this.inlayHintsNS, item.range.end.line, chunks, {}).logError();
            }
        }
    }
...
function activateInlayHints(ctx) {
    const maybeUpdater = {
        ...
        async onConfigChange() {
            ...
            await coc_nvim_1.workspace.nvim.command('hi default link CocRustTypeHint CocHintSign');
            if (this.updater) {
